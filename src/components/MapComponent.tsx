...
export default function MapComponent({ className = '' }: MapProps) {
  ...
  const [activeTab, setActiveTab] = useState<'basemap' | 'wms' | 'statistics' | 'compare'>('basemap')
...
// Handle AI analysis with streaming - now unified with chat
  ...
  const handleLoadComparison = async (params: {
    ...
    try {
      ...
      // Convert bounds to geometry format for API
      const geometry = [
        [sw.lng, sw.lat],
        [ne.lng, sw.lat],
        [ne.lng, ne.lat],
      ...
      const formattedDate1 = params.date1.toISOString().split('T')[0]
      const formattedDate2 = params.date2.toISOString().split('T')[0]
      
      // Create date ranges by adding 30 days to each selected date
      const endDate1 = new Date(params.date1.getTime() + 30 * 24 * 60 * 60 * 1000)
      const endDate2 = new Date(params.date2.getTime() + 30 * 24 * 60 * 60 * 1000)
      
      const formattedEndTime1 = endDate1.toISOString().split('.')[0] + 'Z'
      const formattedEndTime2 = endDate2.toISOString().split('.')[0] + 'Z'
      const formattedStartTime1 = formattedDate1 + 'T00:00:00Z'
      const formattedStartTime2 = formattedDate2 + 'T00:00:00Z'
      ...
      const response = await fetch('http://localhost:5000/api/compare', {
        ...
        body: JSON.stringify({
          ...
          endTime2: formattedEndTime2,
          layer: params.layer,
          cloud_percentage: params.cloudPercentage
        })
      })
      ...
      alert('Failed to load comparison data. Please make sure Flask API is running on localhost:5000')
    } finally {
