...
export default function MapComponent({ className = '' }: MapProps) {
  ...
  const handleAnalyzeWithAI = async () => {
    ...
    try {
      ...
      // Build detailed context if data is available
      if (imageMetadata) {
        analysisContext = `Analyze this satellite imagery with the following parameters:
        
📊 **Data Layer**: ${imageMetadata.layer}
📅 **Date Range**: ${imageMetadata.startDate} to ${imageMetadata.endDate}
...
- Any notable features or patterns in the selected area`
        ...
        analysisContext = `I have drawn an area on the map but haven't loaded satellite data yet. 

Please analyze what you can see from the map context and provide guidance on:
- What type of satellite data would be most useful for this area
- What environmental features appear to be present
- Recommended analysis approaches for this region
- Key spectral indices that would provide valuable insights`
      } else {
        analysisContext = "I haven't drawn an area or loaded data yet. Please provide guidance on satellite imagery analysis and remote sensing best practices for environmental monitoring."
      }
...
🗺️ **Area Coordinates**: 
- Southwest: ${sw.lat.toFixed(4)}, ${sw.lng.toFixed(4)}
- Northeast: ${ne.lat.toFixed(4)}, ${ne.lng.toFixed(4)}
- Approximate area coverage for analysis`
      }

      console.log('Sending to AI with context:', analysisContext)
      ...
      setAiResponse('Failed to connect to AI service. Please make sure Flask API is running on localhost:5000 and API key is configured.')
    } finally {
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="prose prose-sm max-w-none">
                          <p className="text-gray-700 leading-relaxed whitespace-pre-wrap bg-white/60 rounded-lg p-4 border border-gray-200">
                            {aiResponse}
                          </p>
                        </div>
                      </div>
