...
// Handle AI analysis with streaming - now unified with chat
  ...
  const handleLoadComparison = async (params: {
    ...
    try {
      ...
      const formattedDate1 = params.date1.toISOString().split('.')[0] + 'Z'
      const formattedDate2 = params.date2.toISOString().split('.')[0] + 'Z'

      // Call the comparison API endpoint
      const response = await fetch('http://localhost:5000/api/comparison/download', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        ...
        setComparisonData({
          ...
          date1: params.date1.toLocaleDateString(),
          date2: params.date2.toLocaleDateString(),
          layer: params.layer
        })
