...
export default function MapCore({ 
  ...
  // Clear WMS layers (image overlays) and drawn shapes
  const clearWmsLayers = () => {
    ...
    try {
      ...
      if (imageOverlays.length === 0) {
        alert('No satellite data or drawn shapes to clear')
        return
      }

      // Remove all image overlay layers
      imageOverlays.forEach((overlay) => {
        const img = overlay as HTMLImageElement
        if (img.src && img.src.includes('copernicus.eu')) {
          overlay.remove()
      ...
      // Clear any WMS tile layers by checking their URLs
      mapInstanceRef.current.eachLayer((layer: any) => {
        if (layer._url && layer._url.includes('copernicus.eu')) {
          mapInstanceRef.current?.removeLayer(layer)
        }
      })

      // Also clear drawn shapes (polygons, rectangles, circles)
      if (drawnItemsRef.current) {
        drawnItemsRef.current.clearLayers()
      }

      console.log('WMS layers and drawn shapes cleared successfully')
      
    } catch (error) {
      console.error('Error clearing WMS layers:', error)
      alert('Failed to clear satellite data and drawn shapes')
    }
  }
