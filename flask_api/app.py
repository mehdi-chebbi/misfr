...
def download_comparison_images():
    """
    Download and save comparison images for two different dates.
    ...
    try:
        ...
        # For date1 - use single date instead of range
        image1_url = f"{base_wms_url}?" \
            ...
            f"&TIME={date1}&MAXCC={cloud_percentage}"
        
        # For date2 - use single date instead of range
        image2_url = f"{base_wms_url}?" \
            f"SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS={layer}" \
            f"&BBOX={bbox}&CRS=EPSG:4326&WIDTH=2500&HEIGHT=2500&FORMAT=image/png" \
            f"&TIME={date2}&MAXCC={cloud_percentage}"
