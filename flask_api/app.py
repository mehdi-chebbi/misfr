...
VISION_API_KEY = "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"  # <-- CHANGE THIS TO YOUR ACTUAL API KEY
...
def analyze_satellite_image():
    ...
    Expected JSON payload:
    {
        ...
        "address_details": {"city": "Tamanrasset", "country": "Algeria"}
    }
    """
    try:
        vision_api_instance = get_vision_api()
...
def download_comparison_images():
    """
    Download and save comparison images for two different date ranges.
    
    Expected JSON payload (new format):
    {
        "layer": "NDVI-L2A",
        "date1": "2024-01-15T00:00:00Z",
        "endDate1": "2024-02-15T00:00:00Z",
        "date2": "2024-02-20T00:00:00Z", 
        "endDate2": "2024-03-20T00:00:00Z",
    ...
    try:
        ...
        if 'endDate1' in data and 'endDate2' in data:
            # New format with date ranges
            required_fields = ['layer', 'date1', 'endDate1', 'date2', 'endDate2', 'bbox', 'cloudPercentage']
            for field in required_fields:
                if field not in data:
                    logger.error(f"Missing field: {field}")
                    return jsonify({
            ...
            layer = data['layer']
            date1 = data['date1']
            end_date1 = data['endDate1']
            date2 = data['date2']
            end_date2 = data['endDate2']
            bbox = data['bbox']
            cloud_percentage = data['cloudPercentage']
            
            # Use date ranges in URLs
            time1 = f"{date1}/{end_date1}"
            time2 = f"{date2}/{end_date2}"
            
        else:
            # Old format for backward compatibility
            required_fields = ['layer', 'date1', 'date2', 'bbox', 'cloudPercentage']
            for field in required_fields:
                if field not in data:
                    logger.error(f"Missing field: {field}")
                    return jsonify({
            ...
            # Use single dates for backward compatibility
            time1 = date1
            time2 = date2
        
        # Create comparison-images directory if it doesn't exist
        comparison_dir = os.path.join(os.path.dirname(__file__), 'comparison-images')
        os.makedirs(comparison_dir, exist_ok=True)
        
        # Generate unique filenames
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        image1_filename = f"image1_{timestamp}.png"
        image2_filename = f"image2_{timestamp}.png"
        image1_path = os.path.join(comparison_dir, image1_filename)
        image2_path = os.path.join(comparison_dir, image2_filename)
        
        # Construct WMS URLs for both dates
        base_wms_url = "https://sh.dataspace.copernicus.eu/ogc/wms/2e44e6fc-1f1c-4258-bd09-8a15c317f604"
        
        # For date1 range
        image1_url = f"{base_wms_url}?" \
            f"SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS={layer}" \
            f"&BBOX={bbox}&CRS=EPSG:4326&WIDTH=2500&HEIGHT=2500&FORMAT=image/png" \
            f"&TIME={time1}&MAXCC={cloud_percentage}"
        
        # For date2 range
        image2_url = f"{base_wms_url}?" \
            f"SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&LAYERS={layer}" \
            f"&BBOX={bbox}&CRS=EPSG:4326&WIDTH=2500&HEIGHT=2500&FORMAT=image/png" \
            f"&TIME={time2}&MAXCC={cloud_percentage}"
        
        logger.info(f"Downloading comparison images:")
        logger.info(f"Image 1 URL: {image1_url}")
        logger.info(f"Image 2 URL: {image2_url}")
        ...
        success1 = download_image(image1_url, image1_path)
        success2 = download_image(image2_url, image2_path)
        
        if not success1 or not success2:
            # Clean up any partially downloaded files
            for filepath in [image1_path, image2_path]:
                if os.path.exists(filepath):
                    os.remove(filepath)
            
            return jsonify({
                'success': False,
                'error': 'Failed to download one or both comparison images'
            }), 500
        
        # Create accessible URLs for the frontend
        # Since Flask serves from the flask_api directory, we need relative paths
        image1_url_frontend = f"http://localhost:5000/comparison-images/{image1_filename}"
        image2_url_frontend = f"http://localhost:5000/comparison-images/{image2_filename}"
        
        logger.info(f"Comparison images downloaded successfully:")
        logger.info(f"Image 1: {image1_url_frontend}")
        logger.info(f"Image 2: {image2_url_frontend}")
        
        return jsonify({
            'success': True,
            'image1_url': image1_url_frontend,
            'image2_url': image2_url_frontend,
            'date1': date1,
            'date2': date2,
            'time_range1': time1,
            'time_range2': time2,
        ...
        logger.error(f"Error in comparison download endpoint: {e}")
        return jsonify({
            'success': False,
            'error': f'Internal server error: {str(e)}'
        }), 500

# Serve static files from comparison-images directory
@app.route('/comparison-images/<filename>')
def serve_comparison_image(filename):
    ...
    try:
        ...
        logger.error(f"Error serving comparison image {filename}: {e}")
        return jsonify({
            'success': False,
